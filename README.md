...
Выбираете ваш сайт в дашборде и на первой же вкладке Overview ZONE_ID будет справа. Если не нашли, можете просто запустить скрипт `1_get_zone_id.sh`. Из ответа извлекаем id зоны, которая расположена в самом начале ответа:

```json
{"result":[{"id":"ZONE_ID",....
```

Копируем значение ZONE_ID и добавляем в `.env`:
